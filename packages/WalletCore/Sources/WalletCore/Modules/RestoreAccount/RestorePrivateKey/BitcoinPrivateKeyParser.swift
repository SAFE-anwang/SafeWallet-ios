...
class BitcoinPrivateKeyParser {
    ...
    static func generateAddress(
        ...
        case .bitcoin:
            return try generateBitcoinAddress(
                from: privateKeyData,
        ...
        case .litecoin:
            ...
            case .p2pkh, .p2wpkhSh:
                let converter = Base58AddressConverter(
                    addressVersion: testNet ? 0x6F : 0x30,
                    addressScriptVersion: testNet ? 0x3A : 0x32
                )
                return try converter.convert(publicKey: publicKey, type: scriptType).stringValue
            case .p2wpkh, .p2tr:
                let converter = SegWitBech32AddressConverter(
                    prefix: testNet ? "tltc" : "ltc",
                    scriptConverter: ScriptConverter()
                )
                return try converter.convert(publicKey: publicKey, type: scriptType).stringValue
            default:
        ...
        case .dogecoin:
            guard scriptType == .p2pkh else {
                throw BitcoinKeyError.addressGenerationFailed
            }
            let converter = Base58AddressConverter(
                addressVersion: testNet ? 0x6F : 0x1E,
                addressScriptVersion: testNet ? 0x3A : 0x16
            )
            return try converter.convert(publicKey: publicKey, type: scriptType).stringValue
        ...
        case .dash:
            ...
            let converter = Base58AddressConverter(
                addressVersion: testNet ? 0x8C : 0x4C,
                addressScriptVersion: testNet ? 0x13 : 0x10
            )
            return try converter.convert(publicKey: publicKey, type: scriptType).stringValue
        ...
        case .bitcoinCash:
            ...
            let converter = CashBech32AddressConverter(prefix: testNet ? "bchtest" : "bitcoincash")
            return try converter.convert(publicKey: publicKey, type: scriptType).stringValue
