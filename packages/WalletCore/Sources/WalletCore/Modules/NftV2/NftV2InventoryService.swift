...
final class NftV2InventoryService {
    ...
    private static let pendingTransferReconcilePageSize = 200
    ...
    func sendController(
        ...
        await NftV2SendModule.viewController(
            asset: asset,
    ...
    func pendingTransferReconcileData(
        chain: NftV2Chain,
        accountId: String,
        ...
        let normalizedHashes = Set(transactionHashes.map { $0.lowercased() })
        var recordsByHash = [String: TransactionRecord]()
        var paginationData: String?

        while recordsByHash.count < normalizedHashes.count {
            let page = await transactionRecordsPage(
                adapter: adapter,
                paginationData: paginationData,
                limit: Self.pendingTransferReconcilePageSize
            )
            ...
            for record in page {
                let hash = record.transactionHash.lowercased()
                if normalizedHashes.contains(hash) {
                    recordsByHash[hash] = record
                }
            }

            guard page.count >= Self.pendingTransferReconcilePageSize,
                  let lastPaginationData = page.last?.paginationRaw
            else {
                break
            }

            paginationData = lastPaginationData
        }
        ...
        let filteredRecords = transactionHashes.compactMap { recordsByHash[$0.lowercased()] }
        return (records: filteredRecords, lastBlockHeight: adapter.lastBlockInfo?.height)
    }

    func pendingTransferImmediateReconcileData(
        chain: NftV2Chain,
        accountId: String,
        transactionHashes: Set<String>
    ) async -> (records: [TransactionRecord], lastBlockHeight: Int?)? {
        ...
        let filteredRecords = records.filter { normalizedHashes.contains($0.transactionHash.lowercased()) }
        return (records: filteredRecords, lastBlockHeight: adapter.lastBlockInfo?.height)
    }

    private func transactionRecordsPage(
        adapter: ITransactionsAdapter,
        paginationData: String?,
        limit: Int
    ) async -> [TransactionRecord] {
        await withCheckedContinuation { continuation in
            let single = adapter.transactionsSingle(
                paginationData: paginationData,
                token: nil,
                filter: .all,
                address: nil,
    ...
    private func allTransactionRecords(adapter: ITransactionsAdapter) async -> [TransactionRecord] {
        await withCheckedContinuation { continuation in
            var disposable: Disposable?
            disposable = adapter.allTransactionsAfter(paginationData: nil).subscribe(
                onSuccess: { records in
                    disposable?.dispose()
                    continuation.resume(returning: records)
                },
    ...
    func pendingTransfers(accountId: String) async -> [NftV2PendingTransferItem] {
        ...
        return items.compactMap { item in
            pendingTransferItem(from: item, account: currentAccount)
        }
    }

    func savePendingTransfer(accountId: String, asset: NftV2Asset, collection: NftV2Collection, amount: Int, transactionHash: String) async {
        ...
        await pendingTransferStore.save(item)
    }

    func removePendingTransfer(accountId: String, chain: NftV2Chain, transactionHash: String) async {
        await pendingTransferStore.remove(
            accountId: accountId,
    ...
    func pendingTransferItem(
        ...
        return NftV2PendingTransferItem(
            id: "\(accountId)|\(asset.chain.rawValue)|\(transactionHash.lowercased())",
            chain: asset.chain,
            collectionId: collection.id,
            collectionName: collection.name,
            asset: NftV2Asset(
            ...
            amount: max(amount, 1),
            transactionHash: transactionHash,
            explorerUrl: transactionAdapter(chain: asset.chain, account: account)?.explorerUrl(transactionHash: transactionHash),
            submittedAt: submittedAt
        )
    }

    private func pendingTransferItem(from item: NftV2PendingTransferStore.Item, account: Account) -> NftV2PendingTransferItem? {
    ...
    func refreshSendCapability(asset: NftV2Asset) async -> NftV2SendCapability {
        ...
        let nftKey = NftKey(account: account, blockchainType: asset.chain.blockchainType)
        guard let adapter = await nftAdapterManager.ensuredAdapterAsync(nftKey: nftKey) else {
            return .blocked(reason: .unavailable)
        }
