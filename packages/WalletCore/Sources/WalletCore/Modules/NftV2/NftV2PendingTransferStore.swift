...
final class NftV2PendingTransferStore {
    ...
    func items(accountId: String) async -> [Item] {
        await withCheckedContinuation { continuation in
            queue.async {
                continuation.resume(returning: self.readCache(accountId: accountId)?.items ?? [])
            }
        }
    }
    ...
    func save(_ item: Item) async {
        await withCheckedContinuation { continuation in
            queue.async {
                var cache = self.readCache(accountId: item.accountId) ?? Cache(items: [])
                ...
                self.write(cache: cache, accountId: item.accountId)
                continuation.resume()
            }
        }
    }

    func remove(accountId: String, chain: String, transactionHash: String) async {
        await withCheckedContinuation { continuation in
            queue.async {
                guard var cache = self.readCache(accountId: accountId) else {
                    continuation.resume()
                    return
                }
                ...
                self.write(cache: cache, accountId: accountId)
                continuation.resume()
            }
        }
    }
