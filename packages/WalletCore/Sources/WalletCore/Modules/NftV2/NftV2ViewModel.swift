...
final class NftV2ViewModel: ObservableObject {
    ...
    private var resolvedPendingTransferIds = Set<String>()
    private var pendingTransferReconcileTasks = [NftV2Chain: Task<Void, Never>]()
    private var pendingTransfersReloadGeneration = 0
    ...
    func onAppear() {
        ...
        Core.shared.nftAdapterManager.ensureAdapters(for: Core.shared.accountManager.activeAccount)
    ...
    func validatedSendController(
        ...
        onSendSuccess: @escaping (String, NftV2Asset, NftV2Collection, String, Int) -> Void,
        onSendFailed: @escaping (String) -> Void
    ) async -> UIViewController? {
        handleAccountChangedIfNeeded()

        guard let requestAccountId = inventoryService.activeAccountId else {
            return nil
        }
        let key = capabilityKey(asset: asset)
        let refreshed = await inventoryService.refreshSendCapability(asset: asset)
        ...
        return await inventoryService.sendController(
            asset: asset,
            onSendSuccess: { txHash, amount in
                onSendSuccess(requestAccountId, asset, collection, txHash.hs.hexString, amount)
            },
    ...
    func isSending(asset: NftV2Asset) -> Bool {
        sendingAssetKeys.contains(capabilityKey(asset: asset)) || effectivePendingTransfers.contains(where: { $0.asset.id == asset.id })
    }
    ...
    func handleSendSuccess(accountId: String, asset: NftV2Asset, collection: NftV2Collection, transactionHash: String, amount: Int) {
        let submittedAt = Date()

        if accountId == inventoryService.activeAccountId,
           let pendingItem = inventoryService.pendingTransferItem(
               accountId: accountId,
               asset: asset,
        ...
        {
            pendingTransfers.removeAll { $0.id == pendingItem.id }
            pendingTransfers.insert(pendingItem, at: 0)
            resolvedPendingTransferIds.remove(pendingItem.id)
        ...
        Task { [weak self] in
            ...
            await inventoryService.savePendingTransfer(
                accountId: accountId,
                asset: asset,
                collection: collection,
                amount: amount,
            ...
            await MainActor.run {
                guard accountId == self.inventoryService.activeAccountId else {
                    self.markSending(asset: asset, sending: false)
                    return
                }

                self.markSending(asset: asset, sending: false)
                self.reload()
                self.refreshAfterSend(asset: asset)
                self.reloadPendingTransfers()
            }
        }
    }
    ...
    func completeSendPresentation(asset: NftV2Asset) {
        ...
        if !effectivePendingTransfers.contains(where: { $0.asset.id == asset.id }) {
    ...
    private func handleAccountChangedIfNeeded() -> Bool {
        ...
        sendCapabilityTasks.values.forEach { $0.cancel() }
        sendCapabilityTasks.removeAll()
        pendingTransferReconcileTasks.values.forEach { $0.cancel() }
        pendingTransferReconcileTasks.removeAll()
        sendFollowUpTask?.cancel()
        sendFollowUpTask = nil
        sendCapabilityByAssetKey.removeAll()
        sendingAssetKeys.removeAll()
        resolvedPendingTransferIds.removeAll()
        pendingTransfersReloadGeneration += 1
        pendingTransfers = []
    ...
    private func reloadPendingTransfers() {
        guard let accountId = inventoryService.activeAccountId else {
            pendingTransfersReloadGeneration += 1
            pendingTransfers = []
            resolvedPendingTransferIds.removeAll()
            syncDisplayedPendingTransfers()
        ...
        pendingTransfersReloadGeneration += 1
        let generation = pendingTransfersReloadGeneration
        ...
        Task { [weak self] in
            ...
            var items = await inventoryService.pendingTransfers(accountId: accountId)
                .sorted { $0.submittedAt > $1.submittedAt }

            let resolvedIds = await resolvePendingTransferIds(items: items, accountId: accountId)

            await MainActor.run {
                guard generation == self.pendingTransfersReloadGeneration,
                      accountId == self.inventoryService.activeAccountId
                else {
                    return
                }

                self.pendingTransfers = items
                self.resolvedPendingTransferIds = self.resolvedPendingTransferIds
                    .union(resolvedIds)
                    .intersection(Set(items.map(\.id)))
                self.syncDisplayedPendingTransfers()
                self.reconcilePendingTransfersIfNeeded(items: items, accountId: accountId)
            }
        }
    }
    ...
    private func reconcilePendingTransfers(chain: NftV2Chain, records: [TransactionRecord], lastBlockHeight: Int?) {
        ...
        let matched = pendingTransfers.filter { pending in
            ...
            return pendingTransferIsApplied(pending) || pendingTransferReturnedToCurrentAccount(pending, record: record)
        }
        ...
        resolvedPendingTransferIds.formUnion(matched.map(\.id))
        refreshPresentation()
        ...
        Task { [weak self] in
            ...
            guard let accountId = self.inventoryService.activeAccountId else {
                return
            }

            for pending in matched {
                await inventoryService.removePendingTransfer(accountId: accountId, chain: pending.chain, transactionHash: pending.transactionHash)
            }
    ...
    private func reconcilePendingTransfersIfNeeded(items: [NftV2PendingTransferItem], accountId: String) {
        let grouped = Dictionary(grouping: items, by: \.chain)

        for chain in NftV2Chain.allCases {
            pendingTransferReconcileTasks[chain]?.cancel()
            pendingTransferReconcileTasks[chain] = nil
        }

        for (chain, chainItems) in grouped where !chainItems.isEmpty {
            pendingTransferReconcileTasks[chain] = Task { [weak self] in
                guard let self else {
                    return
                }

                let result = await inventoryService.pendingTransferReconcileData(chain: chain, accountId: accountId, transactionHashes: Set(chainItems.map(\.transactionHash)))

                await MainActor.run {
                    guard accountId == self.inventoryService.activeAccountId else {
                        self.pendingTransferReconcileTasks[chain] = nil
                        return
                    }
                    ...
                    self.reconcilePendingTransfers(chain: chain, records: result.records, lastBlockHeight: result.lastBlockHeight)
                }
            }
        }
    }

    private func resolvePendingTransferIds(items: [NftV2PendingTransferItem], accountId: String) async -> Set<String> {
        guard !items.isEmpty else {
            return []
        }

        let groupedItems = Dictionary(grouping: items, by: \.chain)
        var resolvedIds = Set<String>()

        for (chain, chainItems) in groupedItems {
            guard let result = await inventoryService.pendingTransferImmediateReconcileData(
                chain: chain,
                accountId: accountId,
                transactionHashes: Set(chainItems.map(\.transactionHash))
            ) else {
                continue
            }

            let transactionByHash = Dictionary(uniqueKeysWithValues: result.records.map {
                ($0.transactionHash.lowercased(), $0)
            })

            for pending in chainItems {
                guard let record = transactionByHash[pending.transactionHash.lowercased()] else {
                    continue
                }

                if record.failed {
                    resolvedIds.insert(pending.id)
                    continue
                }

                guard case .completed = record.status(lastBlockHeight: result.lastBlockHeight) else {
                    continue
                }

                if pendingTransferReturnedToCurrentAccount(pending, record: record) || pendingTransferIsApplied(pending) {
                    resolvedIds.insert(pending.id)
                }
            }
        }
    ...
    private func pendingTransferReturnedToCurrentAccount(_ pending: NftV2PendingTransferItem, record: TransactionRecord) -> Bool {
        if let evmRecord = record as? EvmTransactionRecord {
            return evmPendingTransferReturnedToCurrentAccount(pending, record: evmRecord)
        }

        guard let transferEventsProvider = record as? TransferEventsProvider else {
            return false
        }

        let outgoingMatch = transferEventsProvider.transferEvents.outgoing.contains {
            $0.value.nftUid == pending.asset.nftUid
        }
        guard outgoingMatch else {
            return false
        }

        return transferEventsProvider.transferEvents.incoming.contains {
            $0.value.nftUid == pending.asset.nftUid
        }
    }

    private func evmPendingTransferReturnedToCurrentAccount(_ pending: NftV2PendingTransferItem, record: EvmTransactionRecord) -> Bool {
        guard let account = Core.shared.accountManager.activeAccount,
              let currentAddress = inventoryService.addressContext(account: account, chain: pending.chain)?.address.lowercased(),
              pending.asset.nftUid.blockchainType == record.source.blockchainType
        else {
            return false
        }
        ...
        case let contractCall as ContractCallTransactionRecord:
            return contractCall.outgoingEvents.contains {
                $0.value.nftUid == pending.asset.nftUid && $0.address.lowercased() == currentAddress
            }
        case let externalCall as ExternalContractCallTransactionRecord:
            return externalCall.outgoingEvents.contains {
                $0.value.nftUid == pending.asset.nftUid && $0.address.lowercased() == currentAddress
            }
        default:
            return false
    ...
    private func syncDisplayedPendingTransfers() {
        displayedPendingTransfers = pendingTransfers.filter(pendingTransferAffectsPresentation)
    }

    private func applyPendingTransfers(to collections: [NftV2Collection]) -> [NftV2Collection] {
        let effectivePendingTransfers = effectivePendingTransfers
        guard !effectivePendingTransfers.isEmpty else {
            return collections
        }

        var result = collections

        for pending in effectivePendingTransfers {
    ...
    private var effectivePendingTransfers: [NftV2PendingTransferItem] {
        pendingTransfers.filter(pendingTransferAffectsPresentation)
    }

    private func pendingTransferAffectsPresentation(_ pending: NftV2PendingTransferItem) -> Bool {
        !resolvedPendingTransferIds.contains(pending.id) && !pendingTransferIsApplied(pending)
    }
}
