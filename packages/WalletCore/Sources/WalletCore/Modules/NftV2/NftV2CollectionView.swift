...
private struct NftV2AssetDetailView: View {
    ...
    @State private var liveIsSending = false
            ...
            Group {
                if !asset.canSend {
                    ThemeButton(text: "nft_v2.send.unavailable".localized, style: .secondary, mode: .transparent) {}
                        .disabled(true)
                } else if isSendingValidation || liveIsSending {
    ...
    private func triggerSend() {
        guard !isSendingValidation, !liveIsSending else {
            return
        }
    ...
    private func refreshCapability(triggerRefresh: Bool) {
        ...
        let latestIsSending = isSending()
        if latestIsSending != liveIsSending {
            liveIsSending = latestIsSending
        }
        ...
        capabilityObserverTask = Task {
            for _ in 0 ..< 60 {
                ...
                let shouldStop = await MainActor.run {
                    ...
                    return liveSendCapability != .checking && !isSendingValidation && !liveIsSending
                }
