...
class MarketTwiitterViewModel {
    private let service: MarketTwiitterService
    ...
    init(service: MarketTwiitterService) {
        self.service = service
        subscribe(disposeBag, service.stateObservable) { [weak self] in self?.sync(state: $0) }
        sync(state: service.state)
    }

    private func sync(state: MarketTwiitterService.State) {
        switch state {
        case .loading:
...
extension MarketTwiitterViewModel {
