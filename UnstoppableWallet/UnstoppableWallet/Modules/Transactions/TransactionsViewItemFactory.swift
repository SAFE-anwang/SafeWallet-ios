...
class TransactionsViewItemFactory {
    ...
    func viewItem(item: TransactionsViewModel.Item, balanceHidden: Bool) -> TransactionsViewModel.ViewItem {
        ...
        case let record as EvmTransactionRecord:
            ...
            if let address = record.transaction.to?.eip55 {
                contractAddress = mapped(address: address, blockchainType: record.source.blockchainType)
            }
            iconType = .localIcon(imageName: item.record.source.blockchainType.iconPlain32)
            title = record.transaction.input.flatMap { evmLabelManager.methodLabel(input: $0) } ?? "transactions.unknown_transaction.title".localized
            subTitle = contractAddress ?? "transactions.unknown_transaction.description".localized()
