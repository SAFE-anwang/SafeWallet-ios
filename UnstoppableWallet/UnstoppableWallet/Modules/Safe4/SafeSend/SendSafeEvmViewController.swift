...
class SendSafeEvmViewController: ThemeViewController {
    private let evmKitWrapper: EvmKitWrapper
    private let viewModel: SendWsafeViewModel
    ...
    init(evmKitWrapper: EvmKitWrapper, viewModel: SendWsafeViewModel, availableBalanceViewModel: ISendAvailableBalanceViewModel, amountViewModel: AmountInputViewModel, recipientViewModel: RecipientAddressViewModel) {
    ...
    private func openConfirm(sendData: SendEvmData) {
        guard let viewController = SendEvmConfirmationModule.viewController(evmKitWrapper: evmKitWrapper, sendData: sendData, isEth2safe: true) else {
            return
        }
...
extension SendSafeEvmViewController: SectionsDataSource {
    ...
    func buildSections() -> [SectionProtocol] {
        [
            ...
            Section(
                    id: "recipient",
                    headerState: .text(text: "safe_zone.send.receiver".localized, topMargin: .margin16, bottomMargin: .margin8),
                    rows: [
                        StaticRow(
                                cell: recipientCell,
                                id: "recipient-input",
