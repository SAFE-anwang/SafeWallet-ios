...
struct SafeSwapParameters: Equatable {
    ...
    static func ==(lhs: SafeSwapParameters, rhs: SafeSwapParameters) -> Bool {
...
class SafeSwapFeeService {
    ...
    private let provider: SafeSwapProvider

    private(set) var parameters: SafeSwapParameters
    ...
    init(evmKit: EvmKit.Kit, provider: SafeSwapProvider, gasPriceService: IGasPriceService, coinService: CoinService, parameters: SafeSwapParameters) {
        self.evmKit = evmKit
    ...
    private func onSwap(error: Error, fallibleGasPrice: FallibleData<EvmFeeModule.GasPrices>) {
        ...
        if let error = error as? SafeSwapKit.Kit.SwapError, error == .cannotEstimate {       // retry request fee every 5 seconds if cannot estimate
    ...
    private func sync(swap: SafeSwapKit.Swap, fallibleGasPrice: FallibleData<EvmFeeModule.GasPrices>) {
...
extension SafeSwapFeeService: IEvmFeeService {
