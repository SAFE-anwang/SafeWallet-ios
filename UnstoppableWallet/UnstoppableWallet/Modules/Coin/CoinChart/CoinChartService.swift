...
class CoinChartService {
    ...
    var periodType: HsPeriodType = .byPeriod(.day1) {
        didSet {
    ...
    private func fetchChartInfo() {
        Task { [weak self, marketKit, coinUid, currency, periodType] in
            do {
                let chartPoints = try await marketKit.chartPoints(coinUid: coinUid, currencyCode: currency.code, periodType: periodType).1
