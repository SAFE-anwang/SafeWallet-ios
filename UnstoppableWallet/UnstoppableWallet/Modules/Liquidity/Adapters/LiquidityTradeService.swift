...
//        }
        ...
        Task { [weak self, liquidityProvider] in
            do {
                let swapData = try await liquidityProvider.swapData(tokenIn: tokenIn, tokenOut: tokenOut)
    ...
    @discardableResult private func syncTradeData() -> Bool {
        ...
        do {
            let tradeData = try liquidityProvider.tradeData(swapData: swapData, amount: amount, tradeOptions: settings.tradeOptions)
...
extension LiquidityTradeService {
    ...
    func transactionData(tradeData: TradeData) throws -> TransactionData {
        try liquidityProvider.transactionData(tradeData: tradeData, type: .add)
    }
