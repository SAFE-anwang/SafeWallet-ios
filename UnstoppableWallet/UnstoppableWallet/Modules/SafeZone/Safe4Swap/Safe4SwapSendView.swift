...
struct Safe4SwapSendView: View {
    @StateObject private var viewModel: Safe4SwapSendViewModel
    ...
    init(transactionData: TransactionData, tokenIn: Token, tokenOut: Token, amountIn: Decimal, swapPresentationMode: Binding<PresentationMode>) {
        _viewModel = .init(wrappedValue: Safe4SwapSendViewModel(transactionData: transactionData, tokenIn: tokenIn, tokenOut: tokenOut, amountIn: amountIn))
    ...
    @ViewBuilder private func quoteView(data: Safe4SwapSendViewModel.SendData) -> some View {
        VStack {
            ScrollView {
    ...
    @ViewBuilder private func tokenRow(title: String, token: Token, amount: Decimal, rate: Decimal?, type: SendConfirmField.AmountType) -> some View {
        let field = SendConfirmField.amount(
            title: title,
            token: token,
            coinValueType: .regular(coinValue: AppValue(kind: .token(token: token), value: amount)),
