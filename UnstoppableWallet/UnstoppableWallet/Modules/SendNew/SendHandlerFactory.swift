...
enum SendHandlerFactory {
    static func handler(sendData: SendData) -> ISendHandler? {
        ...
        case let .liquidityAdd(token0, token1, amount0, amount1, provider):
            return LiquidityAddSendHandler.instance(token0: token0, token1: token1, amount0: amount0, amount1: amount1, provider: provider)
