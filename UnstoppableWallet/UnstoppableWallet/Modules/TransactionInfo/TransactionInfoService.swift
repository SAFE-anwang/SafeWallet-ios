...
class TransactionInfoService {
    ...
    init(transactionRecord: TransactionRecord, adapter: ITransactionsAdapter, currencyManager: CurrencyManager, rateService: HistoricalRateService, nftMetadataService: NftMetadataService, balanceHiddenManager: BalanceHiddenManager) {
        ...
        subscribe(disposeBag, adapter.transactionsObservable(token: nil, filter: .all, address: nil).observeOn(MainScheduler.instance)) { [weak self] in self?.sync(transactionRecords: $0) }
        subscribe(disposeBag, adapter.lastBlockUpdatedObservable.observeOn(MainScheduler.instance)) { [weak self] in self?.syncItem() }
