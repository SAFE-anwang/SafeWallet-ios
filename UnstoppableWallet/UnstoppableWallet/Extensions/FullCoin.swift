...
extension Array where Element == FullCoin {
    ...
    mutating func sort(filter: String, isEnabled: (Coin) -> Bool) {
        sort { lhsFullCoin, rhsFullCoin in
            ...
            let lhsMarketCapRank = lhsFullCoin.coin.marketCapRank ?? (lhsFullCoin.coin.uid == safeCoinUid ? 3 : Int.max)
            let rhsMarketCapRank = rhsFullCoin.coin.marketCapRank ?? (rhsFullCoin.coin.uid == safeCoinUid ? 3 : Int.max)
